...
interface ClientProductPageProps {
  productId: string;
  initialData?: Product; // Optional initial data from server
}
...
const ClientProductPage: React.FC<ClientProductPageProps> = ({ 
      ...
      try {
        ...
        if (data.RelatedProducts?.length > 0) {
          const related = await Promise.all(
            data.RelatedProducts.map(async (id: number) => {
        ...
        if (data.Other_products?.length > 0) {
          const others = await Promise.all(
            data.Other_products.map(async (id: number) => {
              const response = await fetch(`/api/products/${id}`);
              return response.ok ? response.json() : null;
            })
          );
          setOtherProducts(others.filter(Boolean));
        }
      } catch (error) {
  ...
  // ... [Keep all your existing rendering logic including:
  // renderStarRating, LoadingState check, and the main return JSX]
  // Just make sure to remove the async/await from the original code
